    ...
    public function relation(Request $request)
    {
     ...
     //   return $dataType;
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type') && $request->input('type') == 'project_belongstomany_user_relationship') {
                $options = $row->details;
                $skip = $on_page * ($page - 1);
                ...
                if ($search) {
                    ...
                    $relationshipOptions = app($options->model)->take($on_page)->skip($skip)
                        ...
                        ->CurrentUserProject()->get();
                } else {
                    $total_count = app($options->model)->count();
                    $relationshipOptions = app($options->model)->take($on_page)->skip($skip)->CurrentUserProject()->get();
                }
